...
parsedData.selectedIp = selectedIp;
     ...
     setExporting(true);
    axios.post('http://webapp.smartskills.tn/AppGenerator/backend/api/ImportAll',parsedData)
    .then((response) => {
 ...
 console.log(selectedIdsJSON);
     setExporting(true);
    axios.post('http://webapp.smartskills.tn/AppGenerator/backend/api/ExportAll',selectedIdsJSON)
    .then((response) => {
      if(response.data.status===200){
