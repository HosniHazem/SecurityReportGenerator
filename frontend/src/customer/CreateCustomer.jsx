...
function CreateCustomer() {
  ...
  useEffect(() => {
    axios.get(`http://webapp.preprod.ssk.lc/AppGenerator/backend/api/Customer`,).then((res) => {
      if(res.status === 200){
  ...
  const handleDelete = async (e,id) => {
    ...
    e.preventDefault();
     await axios.delete(`http://webapp.preprod.ssk.lc/AppGenerator/backend/api/Customer/${id}/delete`).then(res=>{
      if(res.status === 200)
        {
