...
function UpdateProject() {
  ...
  const { id } = useParams();
    ...
    const [Fich, setFich] = useState(null);
      useEffect(() => {
        axios.get(`http://webapp.smartskills.local/AppGenerator/backend/api/Project/${id}/show`).then((res) => {
...
console.log(data);
      axios.put(`http://webapp.smartskills.local/AppGenerator/backend/api/Project/${id}/update`, data).then(res=>{
          if(res.data.status === 200)
          {
