...
function UpdateProject() {
  ...
  const { id } = useParams();
    ...
    const [Fich, setFich] = useState(null);
      useEffect(() => {
        axios.get(`http://webapp.smartskills.tn/AppGenerator/backend/api/Project/${id}/show`).then((res) => {
    ...
    useEffect(() => {
      axios.get(`http://webapp.smartskills.tn/AppGenerator/backend/api/Customer`,).then((res) => {
        if(res.status === 200){
...
console.log(data);
      axios.put(`http://webapp.smartskills.tn/AppGenerator/backend/api/Project/${id}/update`, data).then(res=>{
          if(res.data.status === 200)
          {
